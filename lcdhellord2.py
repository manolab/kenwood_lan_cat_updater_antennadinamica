...
def update_lcd(host, lcd):
    ...
    if cf != 'ERR':
        ...
        lcd.print(cf.rjust(5) + 'kHz')
        lcd.set_cursor_pos(1, 0)
        lcd.print('CAT: '+read_cat_status(host)+' Controlled: '+read_cat_on(host))
        lcd.set_cursor_pos(2, 0)
        cl = read_current_len(host)
        tl = read_target_len(host)
        lcd.print('Drv: ' + cl[0].rjust(5) + 'm ')
        lcd.set_cursor_pos(3, 0)
        lcd.print('Ref: ' + cl[1].rjust(5) + 'm ')
        st485 = read_485_status(host)
        if st485 != 'ERR':
            if int(st485) > 0:
                lcd.set_cursor_pos(2, 12)
                if tl[0] != cl[0]:
                    lcd.write(126)
                    lcd.print(' ' + tl[0].rjust(5) + 'm')
                else:
                    lcd.print('        ')
                lcd.set_cursor_pos(3, 12)
                if tl[1] != cl[1]:
                    lcd.write(126)
                    lcd.print(' ' + tl[1].rjust(5) + 'm')
                else:
                    lcd.print('        ')
            else:
                lcd.set_cursor_pos(2, 12)
